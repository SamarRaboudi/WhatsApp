...
const CreateGroup = (props) => {
  ...
  const [uriLocal, setUriLocal] = useState(null);
  ...
  const addGroup = async () => {
    try {
      ...
      const newGroup = {
        ...
        creatorId: currentid, // Add creatorId to the new group
      };

      await refGroups.child(key).set(newGroup);
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.membersContainer}>
          <Text style={{ marginBottom: 20, color: 'gray', fontSize: 16 }}>Add members:</Text>
          <FlatList
            style={{ width: '100%' }}
            data={members}
